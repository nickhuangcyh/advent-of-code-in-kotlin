...
fun main() {
    fun part1(input: List<String>): Int {
        ...
        for (command in input) {
            val directionCommand = DirectionCommand.from(command)
    ...
    fun part2(input: List<String>): Int {
        ...
        for (command in input) {
            val directionCommand = DirectionCommand.from(command)
            when (directionCommand) {
                is DirectionCommand.Forward -> {
                    horizontalPosition += directionCommand.value
                    depth += directionCommand.value * aim
                }
...
sealed class DirectionCommand {
    class Forward(val value: Int): DirectionCommand()
    class Down(val value: Int): DirectionCommand()
    class Up(val value: Int): DirectionCommand()

    companion object {
        fun from(command: String): DirectionCommand? {
            val commandList = command.split(" ")
            val direct = commandList.first()
            val value = commandList.last().toInt()
